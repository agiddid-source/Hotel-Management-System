...
const Rooms = {
      ...
      <div class="bg-surface-card border border-outline rounded-xl overflow-visible card-diffusion">
        <div class="flex items-center justify-between px-6 py-4 border-b border-outline">
          <div>
            ...
            <p class="text-sm text-content-muted mt-0.5">${this.data.length} total rooms</p>
          </div>
   ...
   */
  filterRooms(typeId) {
    console.log(`filterRooms called with typeId: ${typeId}`);
    let roomsToRender = [];
    
    if (typeId === 'all') {
      roomsToRender = this.data;
      console.log(`Rooms to render for 'all types': ${roomsToRender.length}`);
    } else {
      roomsToRender = this.data.filter(room => room.roomType.id === typeId);
      console.log(`Rooms to render for type '${typeId}': ${roomsToRender.length}`);
    }
    
    const roomsTableBody = document.getElementById('rooms-table-body');
    if (roomsTableBody) {
      roomsTableBody.innerHTML = this.renderRoomRows(roomsToRender);
      this.bindDropdownEvents(); // Re-bind dropdown events after filtering
    }
